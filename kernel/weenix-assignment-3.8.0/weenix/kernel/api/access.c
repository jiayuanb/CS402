...
int copy_to_user(void *uaddr, const void *kaddr, size_t nbytes)
{
        ...
        // from kaddr to uaddr
        int ret = vmmap_write(curproc->p_vmmap, uaddr, kaddr, nbytes);
        if(ret){
                return ret;
        }
        return 0;
...
addr_perm(struct proc *p, const void *vaddr, int perm)
{
        ...
        vmarea_t *curvma = vmmap_lookup(p->p_vmmap, vpn);
        if(curvma == NULL){
                return 0;
        }
        else{
                if((curvma->vma_prot & perm) == perm){
                        return 1;
                }
        }
        return 0;
...
range_perm(struct proc *p, const void *avaddr, size_t len, int perm)
{
        ...
        uint32_t last_vfn = ADDR_TO_PN((uint32_t) avaddr + len);
        uint32_t start_vfn = ADDR_TO_PN((uint32_t) avaddr);
        vmarea_t *curvma;
        list_iterate_begin(&p->p_vmmap->vmm_list, curvma, vmarea_t, vma_plink){
                if(curvma->vma_start > start_vfn){
                        return 0;
                }
                if(curvma->vma_start <= start_vfn && curvma->vma_end > start_vfn){
                        start_vfn = curvma->vma_end;
                }

                if(start_vfn > last_vfn){
                        return 1;
                }
        } list_iterate_end();
        return 0;
