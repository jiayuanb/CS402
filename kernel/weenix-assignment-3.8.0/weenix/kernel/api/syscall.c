...
sys_write(write_args_t *arg)
{
        // NOT_YET_IMPLEMENTED("VM: sys_write");
        write_args_t kstr;
        if (copy_from_user(&kstr, arg, sizeof(kstr)) < 0) {
        ...
        size_t to_write = kstr.nbytes;
        while(to_write){

                size_t write_size = MIN(PAGE_SIZE, to_write);
                int ret = copy_from_user(buf, kstr.buf, write_size);
                if(ret < 0){
                        curthr->kt_errno = -ret;
                        page_free(buf);
                        return -1;
                ...
                if((ret = do_write(kstr.fd, buf, MIN(PAGE_SIZE, write_size))) < 0){
                        curthr->kt_errno = -ret;
                        page_free(buf);
                        return -1;
                }
...
sys_getdents(getdents_args_t *arg)
{
        ...
        getdents_args_t kstr;
        if (copy_from_user(&kstr, arg, sizeof(kstr)) < 0) {
                curthr->kt_errno = EFAULT;
                return -1;
        }
        uint32_t num_dirents = kstr.count / sizeof(dirent_t);
        
        int total = 0;
        dirent_t temp;
        dirent_t *curdirp = kstr.dirp;

        for(uint32_t i = 0; i < num_dirents; i++){

                int ret = do_getdent(kstr.fd, &temp);
                if(ret < 0){
                        curthr->kt_errno=-ret;
                        return -1;
                }
                ...
                copy_to_user(curdirp+i, &temp, ret);
                total+=ret;
        }
