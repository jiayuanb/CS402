...
proc_create(char *name)
{
        ...
        //                 //dbg_print("fd_pointer %p", process->p_files[fd]);
        ...
        //         for (int fd = 0; fd < NFILES; fd++) {
        //                 //dbg_print("fd_pointer %p", process->p_files[fd]);
        //                 process->p_files[fd] = curproc->p_files[fd];
        //         }
        // }
        for (int fd = 0; fd < NFILES; fd++) {
                //dbg_print("fd_pointer %p", process->p_files[fd]);
...
proc_cleanup(int status)
{
        ...
        dbg_print("finish cleaning up all open files, next clean p_cwd\n");
        if(curproc->p_cwd){
                vput(curproc->p_cwd);
        }
...
do_waitpid(pid_t pid, int options, int *status)
{
        ...
        dbg_print("in do_waitpid pid %d wait for %d\n", curproc->p_pid, pid);
        ...
        if(pid == -1){
                while(1){
                        ...
                        list_iterate_begin(&cur->p_children, child_proc, proc_t, p_child_link) {
                                if(child_proc->p_state == PROC_DEAD){
                                        ...
                                        if(status) *status = child_proc->p_status;
                        ...
                        dbg_print("do wait sched sleep\n");
                        sched_sleep_on(&(cur->p_wait));
                        dbg_print("come back to do wait\n");
                        dbg(DBG_PRINT, "(GRADING1A)\n");
                ...
                do{
                        proc_t * child_proc;
                        // dbg_print("do wait pid = %d by pid = %d", lookup->p_pid, curproc->p_pid);
...
do_exit(int status)
{
        ...
        dbg_print("do_exit curproc pid=%d\n", curproc->p_pid);
        curproc->p_status = status;
        kthread_t* child_thread;
