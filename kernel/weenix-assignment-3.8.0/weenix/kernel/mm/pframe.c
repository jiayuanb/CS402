...
pframe_alloc(mmobj_t *o, uint32_t pagenum)
{
        ...
        pf->pf_pincount = 0;
        // dbg_print("allocate pframe p_addr=%#08x, pincount=%d\n", (uint32_t)pf->pf_addr, pf->pf_pincount);
        list_insert_head(&pframe_hash[hash_page(o, pagenum)], &pf->pf_hlink);

        o->mmo_ops->ref(o);
        o->mmo_nrespages++;

        dbg_print("pframe alloc mmobj=%#08x, mmo_nrespages=%d\n", (uint32_t)o, o->mmo_nrespages);
...
pframe_migrate(pframe_t *pf, mmobj_t *dest)
{
        ...
        if (NULL != pframe_get_resident(dest, pf->pf_pagenum)) {
                ...
                src->mmo_nrespages--;
                dbg_print("pframe alloc mmobj=%#08x, mmo_nrespages=%d\n", (uint32_t)src, src->mmo_nrespages);
                src->mmo_ops->put(src);
                ...
                dbg_print("pframe alloc mmobj=%#08x, mmo_nrespages=%d\n", (uint32_t)dest, dest->mmo_nrespages);
        }
}
...
pframe_fill(pframe_t *pf)
{
        ...
        pframe_set_busy(pf);
        dbg_print("pframe fill page addr %#08x\n", (uint32_t)pf->pf_addr);
        ret = pf->pf_obj->mmo_ops->fillpage(pf->pf_obj, pf);
        dbg_print("finish fill page addr %#08x\n", (uint32_t)pf->pf_addr);
...
{
        // dbg_print("[*] Enter pframe_get %p, %d\n", o, pagenum);
        pframe_t *frame;
restart:
        list_iterate_begin(&(o->mmo_respages), frame, pframe_t, pf_olink) {
                // dbg_print("check pframe pagenum = %d\n", frame -> pf_pagenum);
                if(frame->pf_pagenum == pagenum){
                        ...
                        *result = frame;
                        // dbg_print("[*] Exit pframe_get, return value in loop\n");
        ...
        // dbg_print("cannot find pframe in loop\n");

        while(pageoutd_needed()){
                pageoutd_wakeup();
                // dbg_print("sched out pframe get\n");
                sched_sleep_on(&alloc_waitq);
                // dbg_print("sched back pframe get\n");
        }
        // dbg_print("not need pageoutd\n");

        *result = pframe_alloc(o, pagenum);
        // dbg_print("finish alloc\n");
        if (!(*result)) {
                // dbg_print("cannot alloc\n");
                // sched_make_runnable(curthr);
                // sched_switch();
                // dbg_print("restart to search or alloc");
                goto restart;
        }
        int ret = pframe_fill(*result);
        // dbg_print("[*] Exit pframe_get, allocate a new pframe, fill=%d\n", ret);
        // pframe_pin(*result);
        return ret;
...
pframe_pin(pframe_t *pf)
{
        ...
        ++(pf->pf_pincount);
        // dbg_print("pframe pin p_addr=%#08x, pincount=%d\n", (uint32_t)pf->pf_addr, pf->pf_pincount);
}
...
pframe_unpin(pframe_t *pf)
{
        ...
        // dbg_print("pframe unpin p_addr=%#08x, pincount=%d\n", (uint32_t)pf->pf_addr, pf->pf_pincount);
}
...
{
        // dbg_print("start to clean pframe\n");
        // dbg_print("pframe %#08x is about to clean\n", (uint32_t)pf);
        int ret;
...
pframe_free(pframe_t *pf)
{
        ...
        o->mmo_nrespages--;
        dbg_print("pframe free %#08x, mmo_respages=%d\n", (uint32_t)o, o->mmo_nrespages);
        list_remove(&pf->pf_olink);
